...
from info import ADMINS, PICS_RT, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, DELETE_TIME, CH_FILTER, CH_LINK, UPDATES_CHANNEL, GROUP_SUPPORT
...
# AFB = [[InlineKeyboardButton(text="📎 MR MOVIE LINKZ", url=f'https://t.me/MR_LINK_Z')]]
    ...
    if query.data.startswith("file"):
        ...
        if CUSTOM_FILE_CAPTION:
            try:
                ...
                buttons = [[
                  InlineKeyboardButton(f'©️ {message.chat.title}', 'dupe')
                  ]]
        ...
        if CUSTOM_FILE_CAPTION:
            try:
                f_caption = CUSTOM_FILE_CAPTION.format(file_name='' if title is None else title,
                                                       ...
                                                       file_caption='' if f_caption is None else f_caption)
                buttons = [[
                  InlineKeyboardButton('▫️ Contact Owner', user_id=5172114510)
                  ]]
            except Exception as e:
                logger.exception(e)
                f_caption = f_caption
        ...
        buttons = [[
            InlineKeyboardButton('𝗠𝗔𝗜𝗡 𝗖𝗛𝗔𝗡𝗡𝗘𝗟', url='https://t.me/Mr_MalluOfficial'),
            ],[
            InlineKeyboardButton('𝗠𝗢𝗩𝗜𝗘 𝗥𝗘𝗤𝗨𝗘𝗦𝗧', url=f'https://t.me/mr_malayalm_movies')
            ],[
            InlineKeyboardButton('𝗕𝗢𝗧 𝗨𝗣𝗗𝗔𝗧𝗘𝗦', url='t.me/beta_botz')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(        
        ...
        await query.answer(f"💎 Send Correct Movie Name For Better Results.\nEg - vikram 2022 malayalam \n\n © {query.message.chat.title}", True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
                        [InlineKeyboardButton("◁", callback_data=f"next_{req}_{key}_{offset}"),
             InlineKeyboardButton(text="▷", callback_data=f"next_{req}_{key}_{offset}")]
        )
        btn.insert(0,
            [InlineKeyboardButton(f'©️ {message.chat.title}', 'dupe'),
             InlineKeyboardButton('📞 contact', user_id=5558249587),
 ...
 \n<b>✍️ Note:</b> ミ★ 𝙏𝙝𝙞𝙨 𝙈𝙚𝙨𝙨𝙖𝙜𝙚 𝙬𝙞𝙡𝙡 𝙗𝙚 𝘼𝙪𝙩𝙤-𝙙𝙚𝙡𝙚𝙩𝙚𝙙 𝙖𝙛𝙩𝙚𝙧 5 𝙈𝙞𝙣𝙪𝙩𝙚𝙨 𝙩𝙤 𝘼𝙫𝙤𝙞𝙙 𝘾𝙤𝙥𝙮𝙧𝙞𝙜𝙝𝙩 𝙄𝙨𝙨𝙪𝙚𝙨 ★彡"""
    if imdb and imdb.get('poster'):
        try:
            fmsg = await message.reply_photo(photo='https://graph.org/file/2d64b016dd3b658905981.jpg', caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
        except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty):
            pic = imdb.get('poster')
            poster = pic.replace('.jpg', "._V1_UX360.jpg")
            fmsg = await message.reply_photo(photo='https://graph.org/file/2d64b016dd3b658905981.jpg', caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
        except Exception as e:
            logger.exception(e)
